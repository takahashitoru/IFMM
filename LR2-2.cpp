		  ...
		  if (iSV != jSV){
		    ...
		    // while ((Sigma_i(jSV-1) < 1000*machine_eps || Sigma_j(iSV-1)< 1000*machine_eps) && iSV>0){
		    //    iSV--;
		    //    jSV--;
		    //  }
		    ...
		    // if (Sigma_i(jSV-1) < 1000*machine_eps || Sigma_j(iSV-1)< 1000*machine_eps){
		    // iSV=min(iSV_temp,jSV_temp);
		    // jSV=min(iSV_temp,jSV_temp);
		    // }
		  ...
		  // ANALYZE outfile << "Rank P2P_ij (" << on->id << " " << on->ngbr(iNgbr)->id << " " << on->ngbr(jNgbr)->id << ")" << " " << iSV << endl;
		  // ANALYZE outfile << "Rank P2P_ji (" << on->id << " " << on->ngbr(iNgbr)->id << " " << on->ngbr(jNgbr)->id << ")" << " " << jSV << endl;
		  ...
		  if (NeedUpdate==true){
		    ...
		    // SV_U_comb_i=0;
		    // while (Sigma_U_comb_i(SV_U_comb_i) > epsilon_rel*Sigma_U_comb_i(0) && Sigma_U_comb_i(SV_U_comb_i) > machine_eps && SV_U_comb_i<(Sigma_U_comb_i.rows()-1) ){
		    // // while (Sigma_U_comb_i(SV_U_comb_i) > epsilon && Sigma_U_comb_i(SV_U_comb_i) > machine_eps && SV_U_comb_i<(Sigma_U_comb_i.rows()-1) ){
		    //  SV_U_comb_i++; 
		    // }
		    // if (Sigma_U_comb_i(SV_U_comb_i) > epsilon_rel*Sigma_U_comb_i(0) && Sigma_U_comb_i(SV_U_comb_i) > machine_eps){
		    // // if (Sigma_U_comb_i(SV_U_comb_i) > epsilon && Sigma_U_comb_i(SV_U_comb_i) > machine_eps){
		    ...
		    // while (Sigma_V_comb_j(SV_V_comb_j) > epsilon_rel*Sigma_V_comb_j(0)  && Sigma_V_comb_j(SV_V_comb_j) > machine_eps && SV_V_comb_j<(Sigma_V_comb_j.rows()-1) ){
		    // // while (Sigma_V_comb_j(SV_V_comb_j) > epsilon && Sigma_V_comb_j(SV_V_comb_j) > machine_eps && SV_V_comb_j<(Sigma_V_comb_j.rows()-1) ){
		    //  SV_V_comb_j++; 
		    // }
		    ...
		    // SV_U_comb_j=0;
		    // while (Sigma_U_comb_j(SV_U_comb_j) > epsilon_rel*Sigma_U_comb_j(0) && Sigma_U_comb_j(SV_U_comb_j) > machine_eps && SV_U_comb_j < (Sigma_U_comb_j.rows()-1)){
		    // // while (Sigma_U_comb_j(SV_U_comb_j) > epsilon && Sigma_U_comb_j(SV_U_comb_j) > machine_eps && SV_U_comb_j < (Sigma_U_comb_j.rows()-1)){
		    //  SV_U_comb_j++; 
		    // }
		    ...
		    // SV_V_comb_i=0;
		    // while (Sigma_V_comb_i(SV_V_comb_i) > epsilon_rel*Sigma_V_comb_i(0)  && Sigma_V_comb_i(SV_V_comb_i) > machine_eps && SV_V_comb_i < (Sigma_V_comb_i.rows()-1)){
		    // // while (Sigma_V_comb_i(SV_V_comb_i) > epsilon && Sigma_V_comb_i(SV_V_comb_i) > machine_eps && SV_V_comb_i < (Sigma_V_comb_i.rows()-1)){
		    //  SV_V_comb_i++; 
		    // }
		    // if (Sigma_V_comb_i(SV_V_comb_i) > epsilon_rel*Sigma_V_comb_i(0) && Sigma_V_comb_i(SV_V_comb_i) > machine_eps){
		    // // if (Sigma_V_comb_i(SV_V_comb_i) > epsilon && Sigma_V_comb_i(SV_V_comb_i) > machine_eps){
		    ...
		    // printf("on->ngbr(iNgbr)->rank,SV_U_comb_i: %d %d\n", on->ngbr(iNgbr)->rank,SV_U_comb_i);
		    // printf("on->ngbr(jNgbr)->rank,SV_U_comb_j: %d %d\n", on->ngbr(jNgbr)->rank,SV_U_comb_j);
		    ...
		    // cout << "on->ngbr(jNgbr)->nDOF:" << on->ngbr(jNgbr)->nDOF << endl;
		    // cout << "on->ngbr(jNgbr)->rank:" << on->ngbr(jNgbr)->rank << endl; 
		    // cout << "on->ngbr(jNgbr)->P2M_operator.rows():" << on->ngbr(jNgbr)->P2M_operator.rows() << endl;
		    // cout << "on->ngbr(jNgbr)->P2M_operator.cols():" << on->ngbr(jNgbr)->P2M_operator.cols() << endl;
		    ...
		    if (SV_U_comb_i != SV_V_comb_i){
		      ...
		      // MatrixXd test_U_i = U_recomp_i.block(0,0,on->ngbr(iNgbr)->nDOF,min(SV_U_comb_i,SV_V_comb_i))*R_recomp_U_i.block(0,0,min(SV_U_comb_i,SV_V_comb_i),on->ngbr(iNgbr)->rank+iSV);
		      // MatrixXd diff_U_i = test_U_i - U_comb_i;
		      // printf("diff_U_i.norm(), epsilon_rel*U_comb_i.norm(): %20.5e %20.5e\n",diff_U_i.norm(), epsilon_rel*U_comb_i.norm());

		      // MatrixXd test_V_i = V_recomp_i.block(0,0,on->ngbr(iNgbr)->nDOF,min(SV_U_comb_i,SV_V_comb_i))*R_recomp_V_i.block(0,0,min(SV_U_comb_i,SV_V_comb_i),on->ngbr(iNgbr)->rank+iSV);
		      // MatrixXd diff_V_i = test_V_i - V_comb_i;
		      // printf("diff_V_i.norm(), epsilon_rel*V_comb_i.norm(): %20.5e %20.5e\n",diff_V_i.norm(), epsilon_rel*V_comb_i.norm());
